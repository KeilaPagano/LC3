...
//Con un imdbRating > 8.0


const returnMovieTitle = (watchlist) => {
    const filteredDirector = watchList.filter((movies) => {
        return movies.Director == 'Christopher Nolan';
    });
    const filteredRating = filteredDirector.filter((movies) => {
        return movies.imdbRating > '8.0';
    });

    const mapedTitle = filteredRating.map((titles) => titles.Title);
    return mapedTitle;
};

const titlesArray = returnMovieTitle(watchList);
console.log( "Las peliculas dirigidas por Christopher Nolan con un rating mayor a 8.0 son: ", titlesArray);
